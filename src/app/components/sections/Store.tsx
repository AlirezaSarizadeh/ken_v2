...
function formatPriceIRR(n: number) {
  // ساده و بدون وابستگی: تومان/ریال رو طبق سلیقه‌ت تغییر بده
...
export default function SectionShop({
  ...
  const ui = {
    ...
    sortLabel: t?.ui?.sortLabel ?? "مرتب‌سازی",
    sortPopular: t?.ui?.sortPopular ?? "محبوب‌ترین",
    sortNewest: t?.ui?.sortNewest ?? "جدیدترین",
    sortPriceLow: t?.ui?.sortPriceLow ?? "ارزان‌تر",
    sortPriceHigh: t?.ui?.sortPriceHigh ?? "گران‌تر",
  ...
  const [sort, setSort] = useState<
    "popular" | "newest" | "priceLow" | "priceHigh"
  >("popular");
  ...
  const sorted = [...searched].sort((a, b) => {
    ...
    // popular (Fallback): موجودها اول، بعد بر اساس badge
    const aScore = (a.inStock ? 10 : 0) + (a.badge ? 1 : 0);
    const bScore = (b.inStock ? 10 : 0) + (b.badge ? 1 : 0);
  ...
  return (
    <div
      className="w-full min-h-full flex flex-col items-center justify-center relative py-16 md:py-0 md:my-24"
      id="shop"
    >
      {/* Background */}
          ...
          <div className="w-full mt-6 flex flex-col md:flex-row items-stretch md:items-center gap-3">
            ...
            {/* <div className="relative md:w-64">
              ...
              >
                ...
                <option value="priceLow">{ui.sortPriceLow}</option>
                <option value="priceHigh">{ui.sortPriceHigh}</option>
              </select>
              <div className="absolute inset-y-0 end-4 flex items-center text-red-300/40 pointer-events-none">
                {ui.sortLabel}
              </div>
            </div> */}
          </div>
                ...
                >
                  ...
                  {/* {item.badge && (
                    <div className="absolute top-4 start-4">
                      <div className="px-3 py-1 rounded-full text-[11px] border border-red-700/35 bg-black/40 text-red-100/85">
                        {item.badge}
                      </div>
                    </div>
                  )} */}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-end">
                          ...
                          {/* <div className="mt-1 text-sm text-red-50/90">
                            <span className="font-black">
                              {formatPriceIRR(item.price)}
                            </span>{" "}
                      ...
                      <div className="mt-4 flex items-center gap-2">
                        ...
                        {/* <button
                          ...
                          className={`
                            ...
                            ${
                              item.inStock
                                ? "border-red-700/40 bg-red-900/35 hover:bg-red-900/50 text-white"
                                : "border-white/10 bg-white/5 text-gray-500 cursor-not-allowed"
                            }
                          `}
                          style={{ fontFamily: "'Vazirmatn', sans-serif" }}
                        >
                          {item.inStock ? ui.addToCart : ui.outOfStock}
                        </button> */}
                      </div>
                    </motion.div>
                  </div>
